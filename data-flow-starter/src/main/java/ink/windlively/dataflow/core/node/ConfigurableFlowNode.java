...
import static ink.windlively.dataflow.util.GeneralTools.copyFields;
...
public class ConfigurableFlowNode implements FlowNode {
    ...
    @Override
    @Nullable
    public TransferEntity apply(SourceEntity source, TransferEntity input) throws FlowNodeProcessException {
        TransferEntity target = new TransferEntity();
        GeneralTools.copyFields(input, target);
        ...
        try {
            ...
            if (skipIfException) {
                ...
                throw new FlowNodeProcessException(this.name, ex);
            }
        }
