    ...
    public <R> List<R> select(String sql, Class<R> clazz, boolean nameMapping) {
        try (
            ...
            for (int i = 1; i <= metaData.getColumnCount(); i++) {
                ...
                if (nameMapping) fieldName = GeneralTools.upCaseToCamelCase(fieldName, false);
                String methodName = GeneralTools.genSetterMethodName(fieldName);
            ...
            if (resultSet.next()) {
                ...
                for (int i = 1; i <= metaData.getColumnCount(); i++) {
                    ...
                    ReflectionUtils.invokeMethod(method, r, GeneralTools.conversionService().convert(resultSet.getObject(i), method.getParameterTypes()[0]));
                }
                list.add(r);
            }
            return list;
