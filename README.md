...
## 项目监控页面地址: [https://cygnus.ink/index](https://cygnus.ink/index)
data-flow简单的后台管理页面，支持flow配置可视化查看以及编辑，同步速率监控等，点击右上角或左下角显示导航菜单(由于个人服务器性能原因(1C2G)，当前的页面体验可能较差)
## 开发计划列表
...
```
#### docker-compose集群模式
将`data-flow-server/src/main/docker`目录下的文件复制到docker运行目录，根据自己的环境修改config目录下的application.yml和logback.xml文件，使用如下命令创建容器并启动：
```shell
docker-compose -p data-flow-cluster up
```
有能力者亦可根据自己需求修改部署配置
